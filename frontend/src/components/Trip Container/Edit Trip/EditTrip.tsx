...
const EditTrip = ({ tripId, userId, setPencilClicked }: Props) => {
  ...
  const handleDeleteClick = async () => {
    ...
    try {
      await axios.delete(process.env.REACT_APP_API_DELETETRIP_PRODUCTION!, {
        data,
      });
  ...
  const updateTripName = async (tripId: string, newTripName: string) => {
    try {
      const res = await fetch(process.env.REACT_APP_API_TRIPNAME_PRODUCTION!, {
        method: "PATCH",
        headers: {
