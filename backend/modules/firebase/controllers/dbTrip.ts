...
export const changeTripName = async (newName: string, tripId: string, userId: string) => {
  try {
    if (!userId || !tripId ) {
      throw new Error("Invalid user ID or trip ID");
    }
    ...
    return { message: "Trip name updated in database" };
  } catch (error) {
    console.log(error, 'error in changeTripName: backend/modules/firebase/controllers/dbTrip.ts');
    throw error;
  }
};
