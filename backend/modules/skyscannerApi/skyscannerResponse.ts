import { Cities, FilteredData, FlightInfo, InitialResults } from './types'

const searchList = (filteredData: FilteredData) => {
    ...
    return list.sort((a, b) => Number(a.price) - Number(b.price)); // Convert price to number before subtraction
}

export const manageResponse = (data: InitialResults) => {
    ...
    const result = searchList(filteredData)
    return result
}
